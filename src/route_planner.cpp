...
RoutePlanner::RoutePlanner(RouteModel &model, float start_x, float start_y, float end_x, float end_y): m_Model(model) {
	...
	RouteModel::Node *start = &m_Model.FindClosestNode(start_x, start_y);
...
RouteModel::Node *RoutePlanner::NextNode() {
	std::sort(this->open_list.begin(), this->open_list.end(), [](RouteModel::Node *first, RouteModel::Node *second) { 
      return first->g_value + first->h_value < second->g_value + second->h_value; 
    });
  	
  	RouteModel::Node* firstNode = this -> open_list[0];
    ...
    open_list.push_back(current_node);
  	...
  	while (open_list.size() > 0) {
        current_node = this->NextNode();
      	if (end_node->distance(*current_node) == 0) {
